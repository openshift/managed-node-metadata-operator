...
with open("{}/managed-node-metadata-operator.clusterserviceversion.yaml".format(BUNDLE_BASE_PATH), 'r') as stream:
...
csv['spec']['replaces'] = "managed-node-metadata-operator.v%s" % prev_version
...
csv_filename = "managed-node-metadata-operator.v%s.clusterserviceversion.yaml" % full_version
